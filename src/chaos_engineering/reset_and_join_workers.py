...
cmd_reset1 = f"{ssh_inner} ssh -o StrictHostKeyChecking=no -i /home/kolla/.ssh/k8s_rsa ubuntu@172.16.0.146 'sudo kubeadm reset -f && sudo rm -rf /etc/cni/net.d /etc/kubernetes/kubelet.conf /etc/kubernetes/pki/ca.crt'"
stdin, stdout, stderr = ssh.exec_command(cmd_reset1)
print(stdout.read().decode('utf-8'))

print("Resetting k8s-worker-2...")
cmd_reset2 = f"{ssh_inner} ssh -o StrictHostKeyChecking=no -i /home/kolla/.ssh/k8s_rsa ubuntu@172.16.0.130 'sudo kubeadm reset -f && sudo rm -rf /etc/cni/net.d /etc/kubernetes/kubelet.conf /etc/kubernetes/pki/ca.crt'"
stdin, stdout, stderr = ssh.exec_command(cmd_reset2)
print(stdout.read().decode('utf-8'))
